...
const IncomeSheet = () => {
  ...
  const fetchIncomeSheet = async () => {
    try {
      const response = await axios.post("/account/it-income-sheet/get", {
        selectedYear,
        selectedMonth,
        selectedType,
      });
...
const handleCellChange = useCallback((rowId, key, value) => {
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      const res = await axios.post("/account/it-income-sheet/create", payload);
    ...
    <div className="p-4 overflow-x-auto ">
      <div className="flex justify-between items-center mb-6 border-b pb-3 border-gray-300">
        <h1 className="text-2xl font-extrabold text-gray-900 tracking-tight">
          <span className="text-indigo-600">IT</span> Income Sheet
        </h1>
