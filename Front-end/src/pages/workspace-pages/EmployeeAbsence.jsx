...
const saveDataToServer = async () => {
  ...
  // console.log("Attempting to save ONLY allocation changes:", allocationChangesPayload);
...
const fetchDataFromServer = async () => {
  ...
  if (isLoading) {
    return <div className="p-8 text-center text-lg font-semibold text-indigo-600">Loading Absence Data...</div>;
  }

  if (displayStaffData.length === 0) {
    return <div className="p-8 text-center text-lg font-semibold text-gray-500">No staff data available for tracking.</div>;
  }
    ...
    <div className="bg-white p-4 sm:p-8 font-sans">
      <header className="mb-6 pb-2 border-b border-gray-200">
        ...
        <p className="text-gray-500 mt-2">
          Annual Consolidated Report of Staff Leave Allocations, Usage Patterns, and Work-From-Home History.
        </p>
      </header>
      ...
      <div className="overflow-x-auto shadow-lg rounded-xl border border-gray-100">
        <table className="w-full text-sm text-gray-700 divide-y divide-gray-200">
          ...
          <thead className="bg-white sticky top-0 z-10 border-b-2 border-blue-500">
            <tr>
              <th rowSpan="3" className="px-4 py-3 text-left font-bold text-xs uppercase tracking-wider bg-white sticky left-0 z-20 rounded-tl-xl border-r border-gray-200 text-blue-700" style={{ minWidth: '130px' }}>
                Staff Name
              </th>
              <th colSpan="24" className="px-2 py-2 text-center font-bold text-sm uppercase tracking-wider border-b border-gray-200 bg-gray-50 text-gray-800">
                Monthly Breakdown
              </th>
              <th colSpan="2" rowSpan="2" className="px-4 py-3 text-center font-bold text-xs uppercase tracking-wider bg-blue-50 border-l border-r border-blue-200 text-blue-700">
                Total (Taken)
              </th>
              <th colSpan="2" rowSpan="2" className="px-4 py-3 text-center font-bold text-xs uppercase tracking-wider bg-blue-50 border-r border-blue-200 text-blue-700">
                Balance (Year)
              </th>
              <th colSpan="2" rowSpan="2" className="px-4 py-3 text-center font-bold text-xs uppercase tracking-wider bg-green-100 rounded-tr-xl text-green-700">
            ...
            <tr>
              {MONTHS.map(month => (
                <th key={month} colSpan="2" className="px-1 py-1 text-center font-bold text-xs text-gray-700 border-r border-gray-200 bg-gray-100">
                  {month}
                </th>
              ))}
            </tr>
            ...
            <tr>
              {MONTHS.map(month => (
                <React.Fragment key={month}>
                  <th className="px-1 py-1 text-center font-medium text-xs text-blue-600 border-r border-gray-100 bg-gray-50/70" style={{ minWidth: '35px' }}>
                    WFH
                  </th>
                  <th className="px-1 py-1 text-center font-medium text-xs text-red-600 border-r border-gray-100 bg-gray-50/70" style={{ minWidth: '35px' }}>
                    Leave
                  </th>
                </React.Fragment>
              ))}

              <th className="px-1 py-1 text-center font-medium text-xs text-blue-600 border-r border-blue-200 bg-blue-50" style={{ minWidth: '35px' }}>
                WFH
              </th>
              <th className="px-1 py-1 text-center font-medium text-xs text-red-600 border-r border-blue-200 bg-blue-50" style={{ minWidth: '35px' }}>
                Leave
              </th>
              ...
              <th className="px-1 py-1 text-center font-medium text-xs text-blue-600 border-r border-green-200 bg-green-100" style={{ minWidth: '35px' }}>
                WFH
              </th>
              <th className="px-1 py-1 text-center font-medium text-xs text-red-600 border-r border-green-200 bg-green-100" style={{ minWidth: '35px' }}>
                Leave
              </th>
          ...
          <tbody className="bg-white divide-y divide-gray-100">
            {displayStaffData.map(staff => (
              <tr key={staff.id} className="hover:bg-blue-50/50 transition-colors">
                ...
                {/* Balance (WFH) */}
                <td className={`px-2 py-3 text-center font-bold border-r border-blue-200 ${staff.balanceWfh < 0 ? 'bg-red-500 text-white' : 'bg-teal-50 text-blue-700'}`}>
                  {staff.balanceWfh}
                </td>

                {/* Balance (Leave) */}
                <td className={`px-2 py-3 text-center font-bold border-r border-blue-200 ${staff.balanceLeave < 0 ? 'bg-red-500 text-white' : 'bg-teal-50 text-red-700'}`}>
                  {staff.balanceLeave}
                </td>
                ...
                <td className="p-1 text-center bg-green-100 border-r border-green-200">
                  <EditableCell
                    ...
                    onChange={(value) => handleAllocationChange(staff.id, 'totalAllocationWfh', value)}
                ...
                <td className="p-1 text-center bg-green-100">
                  <EditableCell
                    ...
                    onChange={(value) => handleAllocationChange(staff.id, 'totalAllocationLeave', value)}
